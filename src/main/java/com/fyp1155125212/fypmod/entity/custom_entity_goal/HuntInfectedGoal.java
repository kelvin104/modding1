...
public class HuntInfectedGoal<T extends LivingEntity> extends TargetGoal {
        ...
        public HuntInfectedGoal(MobEntity goalOwnerIn, Class<T> targetClassIn, boolean checkSight) {
            this(goalOwnerIn, targetClassIn, checkSight, false);
        }

        public HuntInfectedGoal(MobEntity goalOwnerIn, Class<T> targetClassIn, boolean checkSight, boolean nearbyOnlyIn) {
            this(goalOwnerIn, targetClassIn, 10, checkSight, nearbyOnlyIn, (Predicate<LivingEntity>)null);
        }

        public HuntInfectedGoal(MobEntity goalOwnerIn, Class<T> targetClassIn, int targetChanceIn, boolean checkSight, boolean nearbyOnlyIn, @Nullable Predicate<LivingEntity> targetPredicate) {
        ...
        public boolean shouldExecute() {
            if (this.targetChance > 0 && this.goalOwner.getRNG().nextInt(this.targetChance) != 0) {
                ...
                return ((this.nearestTarget != null) && ((this.nearestTarget instanceof PlayerEntity)||(this.nearestTarget instanceof AbstractVillagerEntity)) && ((this.nearestTarget.isPotionActive(EffectInit.SICKNESS.get()))||(this.nearestTarget.isPotionActive(EffectInit.SEVERE_SICKNESS.get()))));
            }
        }
