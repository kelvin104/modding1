...
public class ModNearestAttackableTargetGoal<T extends LivingEntity> extends TargetGoal {
    ...
    public ModNearestAttackableTargetGoal(MobEntity goalOwnerIn, Class<T> targetClassIn, boolean checkSight) {
        this(goalOwnerIn, targetClassIn, checkSight, false);
    }

    public ModNearestAttackableTargetGoal(MobEntity goalOwnerIn, Class<T> targetClassIn, boolean checkSight, boolean nearbyOnlyIn) {
        this(goalOwnerIn, targetClassIn, 10, checkSight, nearbyOnlyIn, (Predicate<LivingEntity>) null);
    }

    public ModNearestAttackableTargetGoal(MobEntity goalOwnerIn, Class<T> targetClassIn, int targetChanceIn, boolean checkSight, boolean nearbyOnlyIn, @Nullable Predicate<LivingEntity> targetPredicate) {
    ...
    public boolean shouldExecute() {
        if (this.targetChance > 0 && this.goalOwner.getRNG().nextInt(this.targetChance) != 0) {
            ...
            return (this.nearestTarget != null)&&(!(this.nearestTarget instanceof NeutralCitizen))&&(!(this.nearestTarget instanceof NeutralCitizen_N))&&(!(this.nearestTarget instanceof NeutralCitizen_J));
        }
    }
