...
public class BioNTech extends VaccineItem {
    public BioNTech(Properties p_i48487_1_) {
        super(p_i48487_1_);
    }
    ...
    public void vaccinatePlayer(PlayerEntity player, int time){ //
        if(!(player.isPotionActive(EffectInit.SICKNESS.get()))) {
            player.addPotionEffect(new EffectInstance(EffectInit.VACCINATED_B.get(), time));
